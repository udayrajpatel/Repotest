...
public class CollegeCtl extends BaseCtl {
	...
	public String display(@RequestParam(required = false) Long id, @ModelAttribute("form") CollegeForm form,
		...
		System.out.println("in college method");
		...
		if (id != null && id > 0) {
			CollegeDTO dto = (CollegeDTO) service.findByPK(id);
			System.out.println("dto value" + dto + "...." + id);
			form.populate(dto);
		}
		return "CollegeView";
	}
	...
	public String display(@ModelAttribute("form") CollegeForm form, Model model, Locale locale) {
		...
		List nextlistsize = service.search(new CollegeDTO(), form.getPageNo() + 1, form.getPageSize());
