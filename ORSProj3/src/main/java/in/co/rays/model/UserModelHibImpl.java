...
public class UserModelHibImpl implements UserModelInt {
	...
	public UserDTO authenticate(String login, String password) throws ApplicationException {
		 ...
		 * .createQuery("from Product as p where p.name = ? and p.stocked = ?",
		 * Product.class);
		 */

		/*
		 * public User loginUser(String username, String password) { TypedQuery<User>
		 * query = em.createQuery("Select u from User u " + "where u.username LIKE " +
		 * "?1 AND u.password LIKE " + "?2 AND u.delete =:delete", User.class);
		 * 
		 * query.setParameter(1, username); query.setParameter(2, password);
		 * 
		 * query.setParameter("delete", false); User foundUser =
		 ...
		 */
		/*
		 * String sql1 ="from Student as where s.firstName=:fname and s.lastName=:lname";
		 * 
		 * Query<Student> query = session.createQuery(sql1); query.setParameter("fname",
		 * "John");
		 * 
		 * query.setParameter("lname", "Doe"); theStudents = query.list();
		 * 
		 */
		 ...
		 *  
		 *  List<Student> list = createQuery.setInteger("myage", 21) .setDouble("myscore", 95) .list();
		 *  
		 */

		Query q = session.createQuery("from UserDTO where login >= :name and password <= :pass ");
